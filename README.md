...
## Файлы конфигурации
- **requirements.txt**
  - Содержит список всех Python библиотек, необходимых для работы проекта. Устанавливаются с помощью `pip install -r requirements.txt`.
- **key.pem** и **cert.pem**
  - Ключевые файлы для настройки защищённого соединения через HTTPS. `key.pem` - приватный ключ, `cert.pem` - сертификат SSL/TLS.

## Исходные коды
- **admin.py**
  - Скрипт содержит административные функции приложения, такие как управление пользователями и настройки безопасности.
- **main.py**
  - Основной скрипт Flask приложения, отвечающий за запуск веб-сервера и инициализацию приложения.
- **script.py**
  - Вспомогательный скрипт для выполнения задач по обслуживанию или миграции данных.
- **kor.py**
  - Специализированный скрипт, предназначенный для выполнения уникальных функций в рамках проекта.

## Документация и лицензия
- **README.md**
  - Основной информационный файл проекта, содержит описание проекта, инструкции по установке и использованию.
- **LICENSE.txt**
  - Текст лицензионного соглашения, описывает права и ограничения на использование программного обеспечения.

## Статические файлы и шаблоны
- **static/**
  - Папка содержит статические файлы веб-приложения:
    - **css/**: Стили CSS для визуального оформления страниц.
    - **js/**: JavaScript файлы для интерактивных функций на клиентской стороне.
    - **images/**: Изображения, используемые на веб-страницах.
- **templates/**
  - Шаблоны Jinja2, используемые для генерации HTML страниц на основе данных:
    - **layout.html**: Базовый шаблон, наследуемый другими шаблонами.
...
## Служебные каталоги
- **uploads/**
  - Папка для хранения файлов, загруженных пользователями через веб-интерфейс.
- **__pycache__/**
  - Папка содержит скомпилированные байт-код файлы Python, что ускоряет загрузку модулей при повторном запуске.
- **.git/**
  - Скрытая папка Git, управляет версиями проекта и хранит историю изменений.
- **venv/**
  - Виртуальная среда Python, содержит все необходимые библиотеки и изоляцию зависимостей.
- **.idea/**
  - Папка конфигураций среды разработки PyCharm, включает настройки проекта и конфигурации запуска.
